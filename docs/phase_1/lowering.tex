...
than the sample has no table to go in. Marked, the path folds into the
ancestor it repeats, and both become one table:

\begin{lstlisting}[style=ml]
module Root : sig
  type id
  type t = { id : id; parent_id : id option; idx : int option; text : string }
  val get : id -> t
  val replies : t -> t list
end
\end{lstlisting}

The back-reference and the index are optional here because the same table is
reached two ways: as a document root, which has no parent, and as a reply,
which does. That is the general rule --- a table reached both inside and
outside a collection has an optional back-reference.
