...
\begin{enumerate}
  ...
  \item \textbf{Maps and recursive shapes are not handled.} An object whose
        keys are data rather than schema (\texttt{\{"u\_1001": \{...\}\}})
        currently becomes a column per key. A self-referential shape becomes
        one table per observed depth. The note discusses neither; both need a
        decision before they need an implementation.
